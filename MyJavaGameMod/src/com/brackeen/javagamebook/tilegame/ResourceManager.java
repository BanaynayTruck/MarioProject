    ...
    */
        ...
        for (int y=0; y<height; y++) {
            ...
            for (int x=0; x<line.length(); x++) {
                ...
                // check if the char represents a sprite
                    ...
                    addSprite(newMap, bootSprite, x, y);
                }
                else if (ch == '@') {
    ...
    public void loadCreatureSprites() {
        ...
        for (int i=0; i<4; i++) {
            ...
            for(int x=0; x<enemies;x++)
            	...
            	if(s.getArchType(x).compareTo("balloon")==0)
                	...
                	if(s.getArchType(x).compareTo("monkeykong")==0)
                		enemyAnim[x][i]=createMonkeyKongAnim(
                				images[i][imageIndex++], images[i][imageIndex++], images[i][imageIndex++]);
        }
        ...
        for(int x=0; x<enemies;x++)
            ...
            if(s.getArchType(x).compareTo("monkeykong")==0)
                enemySprites[x]=new MonkeyKong(enemyAnim[x][0], enemyAnim[x][1],
                		enemyAnim[x][2], enemyAnim[x][3]);
    }
    ...
    public String getInvincibleSound()
    {	//return the sound for the warp
    	if(CodeReflection.isTracing() && TilegamePackageTracingEnabled.getTilegamePackageTracingEnabledInstance().isEnabled()) {
        	if(CodeReflection.getAbstactionLevel()>=0)
        	{//check to make sure it's this level of abstraction
    ...
    {	//return the sound for receiving damage, but not dying
    	...
    	return(s.getSoundByReference("pickup"));
    }
    ...
    private Animation createMonkeyKongAnim(Image img1, Image img2,Image img3)
        {
    	if(CodeReflection.isTracing() && TilegamePackageTracingEnabled.getTilegamePackageTracingEnabledInstance().isEnabled()) {
        	if(CodeReflection.getAbstactionLevel()>=0)
        	{//check to make sure it's this level of abstraction
    		...
    		//Scaling image..
    		int scaleAmount = 2;
    		img1 = img1.getScaledInstance(img1.getWidth(null)*scaleAmount,img1.getHeight(null)*scaleAmount, java.awt.Image.SCALE_SMOOTH);
    		img2 = img2.getScaledInstance(img2.getWidth(null)*scaleAmount,img2.getHeight(null)*scaleAmount, java.awt.Image.SCALE_SMOOTH);
    		img3 = img3.getScaledInstance(img3.getWidth(null)*scaleAmount,img3.getHeight(null)*scaleAmount, java.awt.Image.SCALE_SMOOTH);
            Animation anim = new Animation();
            //Normal state
            anim.addFrame(img1, 150);
            //Up anim state
            anim.addFrame(img2, 160);
            //Normal state
            anim.addFrame(img1, 150);
            //down anim state
    ...
    private void loadPowerUpSprites() {
        ...
        // create boot sprite
        anim = new Animation();
        anim.addFrame(loadImage(s.getItemImage("boot",1)),100);
        anim.addFrame(loadImage(s.getItemImage("boot",2)),100);
        anim.addFrame(loadImage(s.getItemImage("boot",3)),100);
        anim.addFrame(loadImage(s.getItemImage("boot",4)),100);
        bootSprite = new PowerUp.Boot(anim);
        
        // create invincibility sprite
        anim = new Animation();
        anim.addFrame(loadImage(s.getItemImage("invincible",1)),1500);
        anim.addFrame(loadImage(s.getItemImage("invincible",2)),100);
        anim.addFrame(loadImage(s.getItemImage("invincible",3)),100);
        anim.addFrame(loadImage(s.getItemImage("invincible",4)),100);
