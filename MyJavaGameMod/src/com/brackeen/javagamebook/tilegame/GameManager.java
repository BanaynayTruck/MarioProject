...
public class GameManager extends GameCore {
    ...
    private boolean powerInvincible = false; //invincible via powerup. By default, false.
    ...
    public void init() {
        ...
        invincibleSound = soundManager.getSound("sounds/"+resourceManager.getInvincibleSound());
        pickupSound = soundManager.getSound("sounds/"+resourceManager.getPickupSound());
        // start music
        if(MUSIC_ON){
...
*/
    ...
    */
    public void update(long elapsedTime) {
        ...
        {//if the player falls out of the bottom of the screen, kill them
                ...
                this.baseScoreMultiplier=1.0f;
                Player.playerJumpSpeedMultiplier = 1;
        		scoreBoard.setMultiplier(this.baseScoreMultiplier);
    ...
    */    
        ...
        long elapsedTime)
    {
        ...
        if (creature instanceof Player) {
            boolean canKill = ((oldY < creature.getY()-0.5) || powerInvincible);
            ...
            if (canKill) {
            	...
            	{	
            		...
            		if(!badguy.isHelper()){//if it's a helper, don't do the following
            			...
            			if(badguy.getHealth()==0 || powerInvincible)
            			{	//The Bad Guy is dead
            				badguy.setState(Creature.STATE_DYING);
            			}else
            		...
            		if (!powerInvincible || badguy.isHelper()) {
        			player.setY(badguy.getY() - player.getHeight());
        			player.jump(true);
            		}
            	}
    ...
    */
    public void acquirePowerUp(PowerUp powerUp) {  	
        ...
        else if (powerUp instanceof PowerUp.Boot) {
        	System.out.println("Aquired!");
			Player player = (Player)map.getPlayer();
			player.playerJumpSpeedMultiplier = (float) 1.3;
			if(SOUND_ON)
        		soundManager.play(pickupSound);
        			...
        			//create timer to schedule music + invinsibility frames
        			Timer timer = new Timer();
        			//pause music if music is on
        			if (MUSIC_ON)
        			midiPlayer.setPaused(true);
        			//play sound if sound is on
        			if (SOUND_ON) {
        				soundManager.play(invincibleSound);
        				soundManager.play(pickupSound);
        			}
        			//become invincible!
        			INVINCIBLE = true;
        			//change background!
        					renderer.setBackground(resourceManager.loadImage("rainbowbackground.PNG"));   
        			TimerTask task = new TimerTask() {

						@Override
						public void run() {
							//... waiting for sound to be over + timing invincibility
							if (MUSIC_ON) {
        		...
        		//in approx. 10 seconds, run task to turn back music + sound
